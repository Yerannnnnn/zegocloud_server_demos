...
if (!(REDIS_HOST && REDIS_PORT && REDIS_USER && REDIS_PSWD)) {
  throw new Error("You must set your REDIS_HOST, REDIS_PORT, REDIS_USER, REDIS_PSWD environment variables.");
}
...
async function _getRoomList() {
  try {
    ...
    return {
      "code": 1,
      "message": `get room list failed ,error:${err}`,
    };
  }
}
